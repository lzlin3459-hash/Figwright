...
FigFlow 在本地运行，用自带的 matplotlib 引擎出图，**不需要、也不会启动 Origin**，因此不产生任何
demo 水印；矢量产物是 SVG（可用 Inkscape/Illustrator 编辑文字与配色），同时给 300 dpi PNG 与 PDF。

## 调用方式

- 一律使用**本 Skill 目录下的 `figflow.cmd`**（安装时已写入本机绝对路径），用绝对路径调用，
  不要让用户自己选 Python。
- 若启动器提示“尚未安装”，读取同目录 `figflow-home.json` 里的 `product_home`，请用户双击该目录下的
  `setup.cmd` 完成一次性安装（仅安装时联网下载开源依赖），装好后再继续。
- 所有命令输出 JSON 到 stdout，便于读取；面向用户时只讲结论，不要直接倾倒原始 JSON。

## 标准流程（新手路径）

1. `figflow.cmd doctor`：确认引擎、依赖、模板就绪（`ready=true`、`origin_required=false`、
   `watermark=false`）。新机器或换 Python 后先跑一次。
2. 用户给文件并表达目的后，运行
   `figflow.cmd recommend "<数据文件>" --intent "<用户的自然语言目的>"`。
...
4. 用户确认后出图：
   `figflow.cmd draw "<数据文件>" --intent "<目的>"`（自动选图）
   或 `figflow.cmd draw "<数据文件>" --template <id>`（用户指定图种）。
   需要在列映射不确定时强制停下来确认，加 `--strict`（此时退出码为 3、`status=needs_confirmation`，
   按返回的 `assignments/reasons` 与用户核对后再画）。
5. 默认在**源文件旁边**生成 `<文件名>_FigFlow_<时间戳>` 文件夹，内含
   `result.png`、`result.svg`、`result.pdf`、源数据副本与 `figflow_report.json`。
...
- FigFlow 只做“展示”，不擅自做平滑、拟合、归一化、去噪、峰指认、统计检验或剔除离群点；
